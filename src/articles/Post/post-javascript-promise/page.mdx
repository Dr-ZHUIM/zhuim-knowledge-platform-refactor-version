...
category: JavaScript
sidecategory: 
abstract: JavaScript 中存在很多异步操作,Promise 将异步操作队列化，按照期望的顺序执行，返回符合预期的结果。可以通过链式调用多个 Promise 达到我们的目的Promise 在各种开源库中已经实现，现在标准化后被浏览器默认支持。promise 是一个拥有 then 方法的对象或函数
cover: 
layer: Post
